...
static int checkMC()
{
    ...
    if (mcID == -1) {
        ...
        mc0_has_folder = 0;
        DIR *mc0_opl_dir = opendir("mc0:PS2L/");
        if (mc0_opl_dir != NULL) {
        ...
        mc1_has_folder = 0;
        DIR *mc1_opl_dir = opendir("mc1:PS2L/");
        if (mc1_opl_dir != NULL) {
...
void checkMCFolder(void)
{
    ...
    snprintf(path, sizeof(path), "mc%d:PS2L/", mcID & 1);
    mkdir(path, 0777);

    snprintf(path, sizeof(path), "mc%d:PS2L/ps2l.icn", mcID & 1);
    ...
    snprintf(path, sizeof(path), "mc%d:PS2L/icon.sys", mcID & 1);
    fd = open(path, O_RDONLY);
    if (fd < 0) {
        fd = openFile(path, O_WRONLY | O_CREAT | O_TRUNC);
        if (fd >= 0) {
